 ...
 *   from "borrowed" → "pending" / "pending_return" (online return request).
  ...
  async (req, res, next) => {
    ...
    try {
      ...
      try {
        ...
        // If role lookup fails, fall back to token role
      }
      ...
      if (!isAdminLike) {
        ...
        // Students can only move their own record into a "pending" return state
        // (legacy "pending" or new "pending_return").
        if (
          desiredStatus &&
